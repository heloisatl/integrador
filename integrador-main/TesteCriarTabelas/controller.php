...
class Controller {
     ...
     */
    public function criarTabela($conn, $nomeTabela, $mvc_campos) {
        $partes = [];
        $temPK  = false;

        foreach ($mvc_campos as $mvc_campo) {
            $nome    = trim($mvc_campo['nome'] ?? '');
            $tipo    = $mvc_campo['tipo']    ?? 'VARCHAR';
            $tamanho = trim($mvc_campo['tamanho'] ?? '');
            $pk      = !empty($mvc_campo['pk']);
            $nn      = !empty($mvc_campo['nn']);
            $ai      = !empty($mvc_campo['ai']);
