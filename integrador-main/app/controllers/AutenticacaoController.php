...
class AutenticacaoController extends Controller
{
    ...
    public function logar(): void
    {
        ...
        if ($resultado) {
            ...
            if ($this->requisicaoJson()) {
                $this->responderJson('sucesso', 'Login realizado com sucesso.', URL_BASE . $destino);
            }
            $this->redirect(URL_BASE . $destino);
        }

        if ($this->requisicaoJson()) {
            $this->responderJson('erro', 'E-mail ou senha inválidos.');
        }
    ...
    public function solicitarRecuperacao(): void
    {
        ...
        if (!$usuario) {
            if ($this->requisicaoJson()) {
                $this->responderJson('erro', 'Se o e-mail existir, você receberá instruções para redefinir a senha.');
            }
    ...
    public function salvarNovaSenha(): void
    {
        ...
        if ($senha !== $confirmacao) {
            if ($this->requisicaoJson()) {
                $this->responderJson('erro', 'As senhas não conferem.');
            }
        ...
        if (empty($_SESSION['password_reset_tokens'][$token]) || $_SESSION['password_reset_tokens'][$token]['expires'] < time()) {
            if ($this->requisicaoJson()) {
                $this->responderJson('erro', 'Token inválido ou expirado. Solicite novo pedido de recuperação.');
            }
        ...
        if ($this->usuarioService->updateSenhaPorEmail($email, $senha)) {
            if ($this->requisicaoJson()) {
                ...
                $this->responderJson('sucesso', 'Senha alterada com sucesso.', URL_BASE . '/login');
            }
            unset($_SESSION['password_reset_tokens'][$token]);
        ...
        $mensagem = $this->usuarioService->getMensagemErro() ?: 'Não foi possível atualizar a senha. Tente novamente.';
        if ($this->requisicaoJson()) {
            $this->responderJson('erro', $mensagem);
        }
    ...
    public function salvarCadastro(): void
    {
        ...
        if ($this->usuarioService->createUsuario($dados)) {
            if ($this->requisicaoJson()) {
                $this->responderJson('sucesso', 'Cadastro realizado com sucesso.', URL_BASE . '/login');
            }
            $_SESSION['flash_sucesso'] = 'Cadastro realizado com sucesso. Faça login para entrar.';
            $this->redirect(URL_BASE . '/login');
        }

        $mensagem = $this->usuarioService->getMensagemErro() ?: 'Não foi possível realizar o cadastro. Verifique os dados informados.';
        if ($this->requisicaoJson()) {
            $this->responderJson('erro', $mensagem);
        }
        $_SESSION['flash_erro'] = $mensagem;
