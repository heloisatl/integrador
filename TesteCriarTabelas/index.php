  ...
  if (!$conn) {
    ...
    if ($dbname) {
      ...
      if ($acao === 'criar_tabela') {
        ...
        $mvc - campos     = $_POST['mvc-campos'] ?? [];

        if ($nomeTabela === '') {
          $feedback = ['type' => 'err', 'msg' => '❌ Informe o nome da tabela.'];
        } else {
          try {
            $sqlGerado = $controller->criarTabela($conn, $nomeTabela, $mvc - campos);
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .mvc-campo-row input:focus,
    .mvc-campo-row select:focus {
      border-color: var(--accent);
    }
...
<body>
    ...
    <?php if ($dbname && $conn): ?>
      <div class="panel">
        ...
        <form method="POST" id="form-criar">
          ...
          <div id="mvc-campos-wrapper"></div>

          <button type="button" class="btn-add" style="margin-top:10px" onclick="adicionarmvc-campo()">
            + Adicionar mvc-campo
          </button>
          ...
          <div style="margin-top:20px">
            ...
            <div class="sql-preview" id="sql-preview">-- preencha os mvc-campos acima</div>
          </div>
  ...
  <script>
    ...
    let contadormvc - campos = 0;

    function adicionarmvc - campo(nome = '', tipo = 'VARCHAR', tamanho = '', pk = false, nn = false, ai = false) {
      const idx = contadormvc - campos++;
      const wrapper = document.getElementById('mvc-campos-wrapper');
      ...
      div.className = 'mvc-campo-row';
      div.id = `mvc-campo-${idx}`;
      div.innerHTML = `
    <input type="text"   name="mvc-campos[${idx}][nome]"    placeholder="nome_coluna"
           value="${nome}" oninput="atualizarPreview()">
    <select name="mvc-campos[${idx}][tipo]" onchange="toggleTamanho(this, ${idx}); atualizarPreview()">
      ${tiposOptions}
    </select>
    <input type="text"   name="mvc-campos[${idx}][tamanho]" placeholder="255"
           value="${tamanho}" id="tam-${idx}" oninput="atualizarPreview()"
           ${SEM_TAMANHO.has(tipo) ? 'disabled style="opacity:.3"' : ''}>
    <div class="checkbox-group">
      <input type="checkbox" name="mvc-campos[${idx}][pk]" ${pk?'checked':''}
             onchange="handlePK(this,${idx}); atualizarPreview()">
      <span>PK</span>
    </div>
    <div class="checkbox-group">
      <input type="checkbox" name="mvc-campos[${idx}][nn]" ${nn?'checked':''}
             onchange="atualizarPreview()">
      <span>NN</span>
    </div>
    <div class="checkbox-group">
      <input type="checkbox" name="mvc-campos[${idx}][ai]" ${ai?'checked':''}
             onchange="atualizarPreview()">
      <span>AI</span>
    </div>
    <button type="button" class="btn btn-danger" onclick="removermvc-campo(${idx})">✕</button>
  `;
      wrapper.appendChild(div);
      atualizarPreview();
    }

    function removermvc - campo(idx) {
      const el = document.getElementById(`mvc-campo-${idx}`);
      if (el) {
        el.style.opacity = '0';
    ...
    function handlePK(cb, idx) {
      ...
      if (cb.checked) {
        const nn = cb.closest('.mvc-campo-row').querySelector(`[name="mvc-campos[${idx}][nn]"]`);
    ...
    function atualizarPreview() {
      ...
      const rows = document.querySelectorAll('#mvc-campos-wrapper .mvc-campo-row');
      const partes = [];
      ...
      const sql = partes.length ?
        ...
        '-- adicione mvc-campos acima';

      document.getElementById('sql-preview').textContent = sql;
    }

    // Carrega bancos via AJAX ao sair do mvc-campo senha
    ...
    // Inicializa com 1 mvc-campo padrão (id PK AI INT) + 1 mvc-campo nome
    <?php if ($dbname && $conn): ?>
      adicionarmvc - campo('id', 'INT', '', true, true, true);
      adicionarmvc - campo('nome', 'VARCHAR', '100', false, true, false);
    <?php endif; ?>
  </script>
